...
# cv2.imwrite('static/sim/trit.png',colorblind_img)
# cv2.waitKey(0)
...
def sim(filename):
    ...
    lms_img = rgb_to_lms(img)
    t_sim_matrix = np.array([[1, 0, 0], [0, 1, 0], [-0.15773032,  1.19465634, 0]], dtype=np.float16)
    lms_img = np.tensordot(lms_img, t_sim_matrix, axes=([2], [1]))
    rgb_img = lms_to_rgb(lms_img)
